    ...
    try:
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                if yawn_label == "Yawn" and yawn_conf >= 0.55:
                    consecutive_yawn_frames += 1
                    
                    if consecutive_yawn_frames > 7:
                        if not is_yawning:
                            is_yawning = True
                            yawn_count += 1
                else:
                    ...
                    if is_yawn_warning_active:
                        is_yawn_warning_active = False
                        yawn_count = 0  

                if yawn_count >= 3:
                    is_yawn_warning_active = True
                
                
                if mouth_bbox is not None:
                    draw_bbox(frame, mouth_bbox, color=(255, 255, 0))
                    
                    x, y = mouth_bbox[0], mouth_bbox[1]
                    
                    if is_yawning:
                        text = f"YAWNING ({yawn_conf*100:.0f}%)"
                        text_color = (0, 0, 255)
                    else:
                        text = "Normal"
                        text_color = (0, 255, 0)
                        
                    cv2.putText(frame, text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2)
            ...
            yawn_display = "Yes!" if is_yawning else "No"
            ...
            response_data = {
                ...
                "trigger_yawn_warning": is_yawn_warning_active  
            }
